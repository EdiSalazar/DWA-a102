...
def P_validty(P):
    '''This function helps to validate if the given precipitation value is
    in the range of validity for the equations (DWA-102)
    '''
    if (P < 500 or P >1700):
        return("Precipitation value is out of the range of validity for the"
                "equation (500-1700 mm/year)")
...
def ETp_validty(ETp):
    ...
    if (ETp < 450 or ETp > 700):
        return("The value of potential evapotranspiration is out of the range"
               "of validity for the equation (450-700 mm/year)")        
...
def greenroof(P, ETp, h, kf = 70, WKmax = 1.0, WP = 0.5):
...
def partpearmsurf(P, ETp, FA = 4, Sp = 1, WKmax = 0.8, WP = 0.65, kf = 36):
    ...
    if (FA < 2 or FA > 6):
        return("The the joint ratio (FA) is out of the range "
               "of validity for the equation (2 - 6 %)")
    if (Sp < 0.1 or Sp > 2):
        return("The value of storage height (Sp) is out of the range "
...
def partpearmsurf2(P, ETp, FA = 8, Sp = 1, WKmax = 0.8, WP = 0.65, kf = 36):
    '''FA corresponds to the joint ratio of the pavers or partially permable
...
def partpearmsurf3(P, ETp, Sp = 3.5, h = 100, kf = 180):
...
def paverstonegrid(P, ETp, FA = 25, Sp = 1, WKmax = 0.8, WP = 0.65):
    '''FA corresponds to the joint ratio of the pavers or partially permable
...
# gravel ground cover
def gravelcover(P, ETp, h = 100, Sp = 3.5, kf = 1.8):
    '''recommended values are:
...
def surfinfiltration(P, ETp, kf, FAsm = "FAsm_suggested"):
...
# Test
print(surfinfiltration(1500, 700, 500))
print(surfinfiltration(1500, 700, 500, 56.4))
print(surfinfiltration(1500, 700, 500, 50))
sum(surfinfiltration(1500, 700, 500, 56.4))
sum(surfinfiltration(1500, 700, 500, 50))

#%% Berechnungsansatz B.4.2: Versickerungsmulde
# Infiltration swale
def infiltswale(P, ETp, kf, FAsm = "FAsm_suggested"):
...
# Test
print(infiltswale(1700, 700, 5000))
print(infiltswale(1700, 700, 500, 6))
print(infiltswale(1700, 700, 500))
sum(infiltswale(1700, 700, 500))
#%% Berechnungsansatz B.4.3: Mulde-Rigolen-Elemente
# Swale-trench element
def swale_trench(P, ETp, kf, FAsm = "FAsm_suggested"):
    '''FAsm (%) stands for percentage of percolation area (swale). The
    recommended value is calculated in terms of the hydraulic
    conductivity (mm/h) as:
    21.86*kf*exp(-0.348)
...
# Swale-trench system
def swale_trench_system(P, ETp, qDr, kf, FAsm = "FAsm_suggested"):
...
def rainwater_usage(P, ETp, VSp, VBr, FABW = 2, qBW = 60):
    '''VSp stands for Specific storage volume (mm) in relation to the connected,
    flow-effective area.
    VBr correspond to the available water volume to use in relation to the
    connected, effective runoff area (mm/d).
    FABW corresponds to proportion of irrigated area in relation to the
    connected area, effective runoff area (-).
    qBW stands for Specific annual requirement for irrigation (l/(m^2*a))
    '''
    if (P < 500 or P > 1700):
        return("Precipitation value is out of the range of validity for the"
               "equation (500 - 1700 mm/year)")        
    ...
    if (FABW < 0 or FABW > 5):
        return("The  proportion of irrigated area in relation to the connected "
               " area is out of the range of validity for the equation (0 - 5)")
    if (qBW < 0 or qBW > 200):
        return("The  Specific annual requirement for irrigation is out of "
               "the range of validity for the equation (0 - 200 l/(m^2*year)")    
    VBw = FABW*qBW
...
def pod_system(P, ETp, Aw, *Azui_aui):
    ...
    if (ETp < 450 or ETp > 700):
        return("The value of potential evapotranspiration is out of the range "
               "of validity for the equation (450 - 700 mm/year)")
    if len(Azui_aui)%2 != 0:
        return("Every area that produce runoff should have its associate "
               "proportion area that produces runoff")
    else:
        partialarea = 0
...
def pod_system(P, ETp, Aw, A_1, a_1, A_2= 0, a_2= 0.0, A_3= 0, a_3= 0.0,
    ...
    if (a_1 < 0.0 or a_1 > 1.0):
        return("Proportion of area (a_1), which directs its runoff to the pond "
               "should be between 0.0 and 1.0")
    if (a_2 < 0.0 or a_2 > 1.0):
        return("Proportion of area (a_2), which directs its runoff to the pond "
               "should be between 0.0 and 1.0")
    if (a_3 < 0.0 or a_3 > 1.0):
        return("Proportion of area (a_3), which directs its runoff to the pond "
               "should be between 0.0 and 1.0")
    if (a_4 < 0.0 or a_4 > 1.0):
        return("Proportion of area (a_4), which directs its runoff to the pond "
               "should be between 0.0 and 1.0")
