...
def steeproof(P, ETp, Sp):
    '''storage (Sp) in roofs varies between 0.1 and 0.6 mm. Recommended storage value
    for steep roof in general is 0.3 mm, in case of glass or metal cover use 0.6 mm
    '''
...
def storageroof(P, ETp, Sp):
    ...
    '''
    a = 0.9121 + 0.000254*P - 0.0003226*ETp - 0.1472*np.log(Sp+1)
    g = 0
    v = 1-a-g
    pfractions = (a, g, v)
    return(pfractions)
...
def partpearmsurf(P, ETp, FA, Sp, WKmax, WP, kf):
    ...
    elements, and its recommended value is 4. The recommended storage heigth (Sp) is 1 mm.
    WKmax corresponds to maximal water capacity (-) and WP to wilting point (-).
    Recommended value for difference (WKmax - WP) is 0.15. 
    kf stands for hydraulic conductivity (mm/h)
    '''
    a = 0.0800734*np.log(P) - 0.0582828*FA - 0.0501693*Sp - 0.385767*(WKmax - WP) + (8.7040284/(11.9086896+kf))
    # DWA-a-102 equation:
    # g = 0.2006 - 0.000253*ETp + 0.05615*FA - 0.0363*np.log(1 + Sp) + 0.1596*np.log(1 + kf) + 0.2778*(WKmax - WP)
    # WABILA equation:
    # g = -0.2006 - 0.000253*ETp + 0.05615*FA - 0.0636*np.log(1 + Sp) + 0.1596*np.log(1 + kf) + 0.2778*(WKmax - WP)
    # in the term 0.2006 was missing the negative value, and the term "- 0.0363"
    # turns to be "- 0.0636", according to with WABILA equations (Henrich et al., 2016)
    v = 0.8529 - 0.1248*np.log(P) + 0.00005057*ETp + 0.002372*FA + 0.1583*np.log(1 + Sp)
    # Due to the discrepancies in the equation of gF. My decision is to apply:
    g = 1-a-v
    pfractions = (a, g, v)
    return(pfractions)
...
def partpearmsurf2(P, ETp, FA, Sp, WKmax, WP, kf):
    ...
    kf stands for hydraulic conductivity (mm/h)
    '''
    a = 0.05912*np.log(P) - 0.02749*FA - 0.03671*Sp - 0.30514*(WKmax - WP) + (4.97687/(4.7975+kf))
...
def partpearmsurf3(P, ETp, Sp, h, kf):
    ...
    # Equation for "v" was modified, because the term "0.123*np.log(h)"
    #  was negative in DWA and should be positive, validated in WABILA
...
def surfinfiltration(P, ETp, BAs, kf):
    '''BAs (%) stands for percentage of percolation area. The recommended value
    is calculated in terms of the hydraulic conductivity (mm/h) as:
    94741*kf*exp(-1.195)
    '''
    a = 0.004264 + 0.001121*np.log(P) - 0.002757*np.log(BAs)
    # g = 0.6207904 + 0.0899322*np.log(P) - 0.0001152*ETp - 0.0719723*np.log(BAs)
    v = 0.3999 - 0.09317*np.log(P) + 0.00009746*ETp + 0.07474*np.log(BAs)
    g = 1 - (a + v)
    pfractions = (a, g, v)
    return(pfractions)
...
# Infiltration swale
def infiltswale(P, ETp, BAsm, kf):
    '''BAsm (%) stands for percentage of percolation area (swale). The recommended
    value is calculated in terms of the hydraulic conductivity (mm/h) as:
    42.323*kf*exp(-0.314)
    '''
    g = 0.8608 + 0.02385*np.log(P) - 0.00005331*ETp - 0.002827*BAsm - 0.000002493*kf + 0.0009514*np.log(kf/BAsm)
    v = 0.000008562*ETp + (2.611/(P-64.35))*BAsm**0.9425 - 0.000001211*kf
    a = 1 - g - v
    pfractions = (a, g, v)
    return(pfractions)
...
def swale_trench(P, ETp, BAsm, kf):
    '''BAsm (%) stands for percentage of percolation area (swale). The recommended 
    value is calculated in terms of the hydraulic conductivity (mm/h) as:
    21.86*kf*exp(-0.348)
    '''
    a = -0.03867 + 0.007684*np.log(P) + 0.000003201*BAsm + 0.0002564*kf - 0.0001187*BAsm*kf + 0.004161*np.log(kf/BAsm)
    # In WABILA manual the term "0.0002365*kf" is positive and not negative
    # as appear in DWA-102
    # g = 0.8803 + 0.01866*np.log(P) - 0.00004867*ETp - 0.001997*BAsm + 0.0002365*kf
    v = 0.000008879*ETp + (2.528/(P-81.65))*BAsm**0.9496 - 0.00007768*kf
    g = 1 - (a + v)
    pfractions = (a, g, v)
    return(pfractions)
...
# Swale-trench system
def swale_trench_system(P, ETp, BAsm, qDr, kf):
    '''BAsm stands for percentage of percolation area (swale), and qDr for
    ...
    '''
    a = 0.8112 + 0.0003473*P - 0.00001845*ETp - 0.04793*BAsm + 0.0007481*qDr - 0.4389*np.log(kf + 1)
    # g = 1.669 - 0.3005*np.log(P) - 0.00006933*ETp + 0.3044*np.log(BAsm) + 0.4581*np.log(kf + 1)
    # In WABILA manual the term "0.01828*np.log(kf + 1)" has a minus not
    # a multiplication a as appears in DWA-102
    v = 0.1428 - 0.02661*np.log(P) + 0.00005668*ETp + 0.0288*np.log(BAsm) - 0.0001825*qDr - 0.01828*np.log(kf + 1)
    g = 1 - (a + v)
    pfractions = (a, g, v)
    return(pfractions)
...
def rainwater_usage(P, ETp, VSp, VBr, VBw1, VBw2):
    ...
    if VBw == 0:
        ...
        v = - 0.0001927*P + 0.0001831*ETp + 0.0006083*VBw - 0.0000003127*VBw**2 - 0.3092*np.exp(3.269/VSp) + (1.424/(2.782 + VBr)) + 0.0001885*Vnmin
        # In WABILA manual the term is "- 0.0001927*P"
    if VBr == 0:
        e = 0
    else:
        e = 0.4451 - 0.0003529*P - 0.00007728*ETp + 0.06821*np.log10(VSp) - 0.0002507*VBw + 0.2349*np.log10(VBr) + 0.0001738*Vnmin
    a = 1 - (v + e)
    pfractions = (a, e, v)
    return(pfractions)
...
def pod_system(P, ETp, ATeich, *Azui_aui):
    '''ATeich stands for pod surface (m2),
    Azui corresponds to the Area i, which directs its runoff to the pond (m2),
    azui corresonds to proportion of area i, which directs its runoff to
    the pond (-)
    '''
    if len(Azui_aui)%2 != 0:
    ...
    v = (ETp*ATeich)/(P*(ATeich + partialarea))
    a = 1 - v
    g = 0
    pfractions = (a, g, v)
    return(pfractions)
...
#%% Berechnungsansatz B.4.6: Teichanlage mit Zufluss von befestigten Flächen
# Pond system with inflow from paved areas
def pod_system(P, ETp, ATeich, Azu_1, azu_1, Azu_2= 0, azu_2= 0.0, Azu_3= 0, azu_3= 0.0, Azu_4= 0, azu_4= 0.0):
    '''ATeich stands for pod surface (m2),
    Azu_i corresponds to the Area i, which directs its runoff to the pond (m2),
    azu_i corresonds to proportion of area i (0.0-1.0), which directs its runoff to
    the pond (-)
    '''   
    v = (ETp*ATeich)/(P*(ATeich + Azu_1*azu_1 + Azu_2*azu_2 + Azu_3*azu_3 + Azu_4*azu_4))
    a = 1 - v
    g = 0
    pfractions = (a, g, v)
    return(pfractions)
